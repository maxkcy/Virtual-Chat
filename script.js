...
function initHeroGrid() {
    const canvas = document.getElementById(HERO_GRID_CANVAS_ID);
    ...
    // Flat grid lying on the ground plane (XZ), orange centre axes + neon grid.
    const grid = new THREE.GridHelper(HERO_GRID_SIZE, HERO_GRID_DIVISIONS, HERO_GRID_CENTER_COLOR, HERO_GRID_LINE_COLOR);
    ...
    if (glowCtx) {
        ...
        const floor = new THREE.Mesh(new THREE.PlaneGeometry(HERO_GRID_SIZE, HERO_GRID_SIZE), new THREE.MeshBasicMaterial({
            map: glowTexture,
            transparent: true,
            depthWrite: false,
        }));
...
function initHeroScene() {
    ...
    const slot = canvas ? canvas.parentElement : null;
    if (!canvas || !slot)
        return;
    ...
    let plane = null; // THREE.Mesh — the logo
    let rafId = 0;
    ...
    let startPitch = 0;
    let moved = 0;
    const sway = (t) => Math.sin(t * 0.35) * 0.45;
    const setNdc = (e, rect) => {
    ...
    const onPointerDown = (e) => {
        ...
        const hits = raycaster.intersectObject(plane);
        if (hits.length === 0)
            return; // only grab when clicking the logo
        dragging = true;
        moved = 0;
    ...
    loader.load(HERO_TEXTURE_SRC, (texture) => {
